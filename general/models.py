...
class GeneralCategory(TimeStampedModel, OrderedModel):
    """
    الكاتيجوري اللي بيضيفها الأدمن من الداشبورد.
    كل كاتيجوري هي بمثابة "تطبيق" مستقل جوه General.
    """
...
class GeneralSkill(TimeStampedModel, OrderedModel):
    """
...
class StudentGeneralProgress(TimeStampedModel):
    """
    تتبع تقدم الطالب في كل skill جوه General.
    """
    student = models.ForeignKey(
        User, on_delete=models.CASCADE,
        related_name='general_progress', verbose_name="الطالب"
    )
    skill = models.ForeignKey(
        GeneralSkill, on_delete=models.CASCADE,
...
class StudentGeneralQuestionAttempt(TimeStampedModel):
    ...
    student = models.ForeignKey(
        User, on_delete=models.CASCADE,
        related_name='general_question_attempts', verbose_name="الطالب"
    )
    skill = models.ForeignKey(
        GeneralSkill, on_delete=models.CASCADE,
