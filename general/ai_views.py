...
def extract_book(request):
    """
    POST /api/general/ai/extract-book/
    Body (multipart): { name, pdf_file }
    """
    from .ai_models import GeneralExtractedBook
    from .tasks import general_extract_book_task
    ...
    try:
        ...
        book = GeneralExtractedBook.objects.create(
            name=name,
        ...
        general_extract_book_task.delay(book.id)
        ...
        logger.error(f"[General extract_book] Error: {str(e)}")
...
def extract_book_status(request, book_id):
    """
    GET /api/general/ai/extract-book/{book_id}/status/
    """
    from .ai_models import GeneralExtractedBook

    book = get_object_or_404(GeneralExtractedBook, id=book_id)
    return Response({
        'book_id': book.id,
        'name': book.name,
        'status': book.status,
...
def list_extracted_books(request):
    """
    GET /api/general/ai/extract-book/
    """
    from .ai_models import GeneralExtractedBook

    books = GeneralExtractedBook.objects.filter(status='DONE').order_by('-created_at')
    return Response({
        'books': [
            {
                'id': b.id,
...
def extract_media(request):
    """
    POST /api/general/ai/extract-media/
    Body (multipart): { name, media_file }
    """
    from .ai_models import GeneralExtractedMedia
    from .tasks import general_extract_media_task
    ...
    try:
        ...
        media = GeneralExtractedMedia.objects.create(
            name=name,
        ...
        general_extract_media_task.delay(media.id)
        ...
        logger.error(f"[General extract_media] Error: {str(e)}")
        return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
def extract_media_status(request, media_id):
    """
    GET /api/general/ai/extract-media/{media_id}/status/
    """
    from .ai_models import GeneralExtractedMedia

    media = get_object_or_404(GeneralExtractedMedia, id=media_id)
    return Response({
        'media_id': media.id,
        'name': media.name,
...
def list_extracted_media(request):
    """
    GET /api/general/ai/extract-media/
    """
    from .ai_models import GeneralExtractedMedia

    media_qs = GeneralExtractedMedia.objects.filter(status='DONE').order_by('-created_at')
    return Response({
        'media': [
            {
                'id': m.id,
...
def generate_skill(request):
    """
    POST /api/general/ai/generate-skill/
    ...
    from .tasks import general_generate_skill_task
    ...
    category = get_object_or_404(GeneralCategory, id=category_id, is_active=True)
    ...
    try:
        ...
        general_generate_skill_task.delay(job.id)
        ...
        logger.error(f"[General generate_skill] Error: {str(e)}")
        return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
def generation_job_status(request, job_id):
    """
    GET /api/general/ai/jobs/{job_id}/status/
    """
    from .ai_models import GeneralAIGenerationJob

    job = get_object_or_404(GeneralAIGenerationJob, id=job_id)
