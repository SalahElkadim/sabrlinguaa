...
class EspExtractedBook(models.Model):
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
    ...
    uploaded_by = models.ForeignKey(
        ...
        related_name='esp_extracted_books'
    )
...
class EspExtractedMedia(models.Model):
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('PROCESSING', 'Processing'),
        ('DONE', 'Done'),
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='esp_extracted_media'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class EspAIGenerationJob(models.Model):
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('PROCESSING', 'Processing'),
        ('DONE', 'Done'),
    ...
    books = models.ManyToManyField(
        EspExtractedBook, blank=True,
        related_name='generation_jobs'
    )
    media = models.ManyToManyField(
        EspExtractedMedia, blank=True,
        related_name='generation_jobs'
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='esp_generation_jobs'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
