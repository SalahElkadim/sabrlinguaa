...
def get_skill_questions(request, skill_id):
    ...
    if skill.skill_type == 'VOCABULARY':
        questions = VocabularyQuestion.objects.filter(
            ielts_skill=skill, usage_type='IELTS', is_active=True
        ...
        # Speaking
        videos = SpeakingVideo.objects.filter(ielts_skill=skill, usage_type='IELTS', is_active=True).prefetch_related('questions').order_by(DIFFICULTY_ORDER, 'order', 'id')
        for video in videos:
            video_questions = []
            for q in video.questions.filter(is_active=True).order_by('order', 'id'):
                video_questions.append({
