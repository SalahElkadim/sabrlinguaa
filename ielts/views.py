...
def create_listening_audio(request):
    """
    إنشاء تسجيل صوتي جديد لمهارة IELTS Listening

    POST /api/ielts/listening/audio/create/
    ...
    data = request.data.copy()
    required_fields = ['title', 'audio_file', 'ielts_skill']
    for field in required_fields:
        if field not in data:
            return Response({field: f'{field} مطلوب'}, status=status.HTTP_400_BAD_REQUEST)
...
def get_skill_questions(request, skill_id):
    ...
    if skill.skill_type == 'VOCABULARY':
        qs = VocabularyQuestion.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        ...
        qs = GrammarQuestion.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        questions = _get_ordered_questions(qs, skill.question_order_type)
        paginator = Paginator(questions, page_size)
        page_obj = paginator.get_page(page)
        for q in page_obj:
        ...
        qs = ReadingPassage.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        ...
        qs = ListeningAudio.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        ...
        qs = SpeakingVideo.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        ...
        qs = WritingQuestion.objects.filter(general_skill=skill, usage_type='IELTS', is_active=True)
        questions = _get_ordered_questions(qs, skill.question_order_type)
        paginator = Paginator(questions, page_size)
        page_obj = paginator.get_page(page)
        for q in page_obj:
...
def update_listening_audio(request, audio_id):
    """
    PUT/PATCH /api/ielts/listening/audio/<audio_id>/update/
    """
    from sabr_questions.models import ListeningAudio
