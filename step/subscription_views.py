...
def step_subscription_status(request):
    """
    GET /api/step/subscription/status/
    بيرجع حالة الطالب: كام سؤال حل، هل عنده subscription، وهل محتاج يشترك
    """
    student = request.user
    solved_count = get_student_solved_count(student)
    is_subscribed = has_active_step_subscription(student)

    # جيب الـ subscription الحالية لو موجودة
    active_sub = STEPSubscription.objects.filter(
        student=student,
        payment_status='paid',
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def list_step_plans(request):
    """
    GET /api/step/subscription/plans/
    بيرجع الخطط المتاحة
    """
    plans = STEPSubscriptionPlan.objects.filter(is_active=True).order_by('price')
    return Response({
        'plans': [
            {
                'id': p.id,
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def initiate_step_payment(request):
    """
    POST /api/step/subscription/pay/
    ...
    plan = get_object_or_404(STEPSubscriptionPlan, id=plan_id, is_active=True)

    # لو عنده اشتراك نشط بالفعل
    if has_active_step_subscription(request.user):
        return Response(
    ...
    callback_url   = f"{settings.FRONTEND_URL}/step/payment/callback"

    try:
        payment_data = create_payment(
            amount_halalas=amount_halalas,
            description=f"اشتراك STEP - {plan.get_plan_type_display()}",
            callback_url=callback_url,
            token=token,
            metadata={
                'plan_id': str(plan.id),
                'student_id': str(request.user.id),
                'source': 'step_subscription',
            }
        )
    except Exception as e:
        logger.error(f"[STEP payment] Moyasar error: {e}")
        return Response(
...
def step_payment_callback(request):
    ...
    try:
        ...
        logger.error(f"[STEP callback] get_payment failed: {e}")
...
def _activate_subscription(subscription: STEPSubscription, plan: STEPSubscriptionPlan):
    now = timezone.now()
